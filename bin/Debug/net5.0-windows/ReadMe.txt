SplashScreen ver. 1.0.0.0 2021-10-08
Written by: Mikael Ulfsparre – Obsecure AB

This application will display a splash screen with the text located in the “Texts.txt” file on all monitors.

Usage:
The application will look for a file called RunSplashScreen.txt in the same folder as the executable. If this file does not exist it will exit the application.

ALL FILENAMES ARE CASE SENSATIVE

Texts.txt
The Text.txt file contains the texts that should be shown and must be located in the same directory as the executable. 
Each line will be one text that are shown. 
...
A text that does not start with a 1: or a 1: will not be displayed.
